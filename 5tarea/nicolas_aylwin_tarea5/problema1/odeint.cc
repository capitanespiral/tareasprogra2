...
matriz<matriz<double>> odeint::euler_cromer(matriz<double> (*g)(double,const matriz<double> &,const matriz<double> &,matriz<double> &),double tf, char c){
  ...
  matriz<matriz<double>> res(5,colum,m);//Filas t,x,v,a. Columnas cada paso (x,v,a son de todos los cuerpos con todas las coordenadas requeridas, por eso matriz de matrices)
  if(c!='u'){//Si quiero todos los resultados guardo el inicial primero
    t_i={t};matriz<double> T(1,t_i);
    res(0,0)=T;res(1,0)=x;res(2,0)=v;res(3,0)=a;res(4,0)=q;
  }
  ...
  while(t_c<=tf){
    ...
    if(c=='u' && t_c>tf){
      t_i={t_c};matriz<double> T(1,t_i);
      res(0,0)=T;res(1,0)=x_c;res(2,0)=v_c;res(3,0)=a;res(4,0)=q;
    } 
  }
  //Devuelvo resultado
  return res;
...
matriz<matriz<double>> odeint::rk4(matriz<double> (*g)(double,const matriz<double> &,const matriz<double> &,matriz<double> &),double tf, char c){
  if(tf==0) tf=t;
  vector<double> t_i;
  ...
  if(c!='u')
    colum=cuentapasos(t,tf,h);
  matriz<matriz<double>> res(5,colum,m);
  if(c!='u'){
    t_i={t};matriz<double> T(1,t_i);
    res(0,0)=T;res(1,0)=x;res(2,0)=v;res(3,0)=a;res(4,0)=q;
  }
...
matriz<matriz<double>> odeint::vel_velver(matriz <double> (*g)(double,const matriz<double> &,const matriz<double> &,matriz<double> &),double tf,char c){
  if(tf==0) tf=t;
  vector<double> t_i;
  int colum=1;
...
matriz<matriz<double>> odeint::pefrl(matriz<double> (*g)(double,const matriz<double> &,const matriz<double> &,matriz<double> &),double tf,char c){
  if(tf==0) tf=t;
  vector<double> t_i;
  int colum=1;
...
//revisar este, esta muy poco eficiente
