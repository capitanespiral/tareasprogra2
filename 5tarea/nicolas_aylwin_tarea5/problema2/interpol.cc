...
//Intepolacion con splines, se entrega una matriz con una columna y n filas, conteniendo en cada una uno de los polinomios del spline.
...
matriz<poli<double>> i_pol::spline(char a){
  ...
  //Contador para mover las columnas
  int j=0;
  ...
  if(a=='n'){
    coef(0,0)=(hi+hi1)/3;coef(0,1)=hi1/6;coef1.push_back(ki1/hi1-ki/hi);
  }
  else{
  ...
  for(int i=1;i<cant-3;++i){
    ...
    coef(i,j)=hi/6;coef(i,j+1)=(hi+hi1)/3;coef(i,j+2)=hi1/6;
    coef1.push_back(ki1/hi1-ki/hi);++j;
  }
   //ultima fila
  ...
  if(a=='n'){
    coef(cant-3,cant-4)=hi/6;coef(cant-3,cant-3)=(hi+hi1)/3;
    coef1.push_back(ki1/hi1-ki/hi);
  }
  else{
