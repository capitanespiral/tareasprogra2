...
//ESTO NO ES NECESARIO REVISARLO, tu tiempo es valioso
//La funcion Jn con n fijo evaluada para todos los x desde "x" avanzando con "paso" hasta "fin".
//Esta funcion no tiene relevancia en lo pedido, solo la use para corroborar que estaba obteniendo los valores correctos
vector<double> Ji_x(double x, float paso, float fin ,int n, int N=-1){
	vector<double> v;
	double j;
	while(x<=fin){
		j=iter_bessel_dec(x,n,N);
		v.push_back(j);
		x+=paso;
	}
	return v;
